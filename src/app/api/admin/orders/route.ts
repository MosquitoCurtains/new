...
export async function POST(request: NextRequest) {
  try {
    ...
    // Fetch cart with project info and journey tracking IDs
    const { data: cart, error: cartError } = await supabase
      .from('carts')
      .select(`
        *,
        projects!project_id (
          id, email, lead_id, customer_id, visitor_id, session_id,
    ...
    // Extract lead contact info from nested join (project → lead)
    const projectLead = (project?.leads as Record<string, unknown>) || null
    ...
    // Create the order — cascade visitor_id/session_id/lead_id from cart/project
    const { data: order, error: orderError } = await supabase
      ...
      .insert({
        ...
        billing_first_name: billing_first_name || projectLead?.first_name as string || null,
        billing_last_name: billing_last_name || projectLead?.last_name as string || null,
        billing_phone: billing_phone || projectLead?.phone as string || null,
        ...
        shipping_first_name: reqShipFirst || cart.shipping_first_name || billing_first_name || projectLead?.first_name as string || null,
        shipping_last_name: reqShipLast || cart.shipping_last_name || billing_last_name || projectLead?.last_name as string || null,
        ...
        visitor_id: cart.visitor_id || (project?.visitor_id as string) || null,
        session_id: cart.session_id || (project?.session_id as string) || null,
        lead_id: cart.lead_id || (project?.lead_id as string) || null,
      })
      .select('*')
      .single()
    ...
    // Fire journey event: purchase_completed
    const orderVisitorId = cart.visitor_id || (project?.visitor_id as string) || null
    const orderSessionId = cart.session_id || (project?.session_id as string) || null
    const orderLeadId = cart.lead_id || (project?.lead_id as string) || null

    supabase
      .from('journey_events')
      .insert({
        visitor_id: orderVisitorId,
      ...
      .then(({ error: evtErr }) => {
        if (evtErr) console.error('Error firing purchase_completed journey event:', evtErr)
      })
      ...
      try {
        ...
        if (lead) {
          ...
          if (existingCustomer) {
            ...
            // Create new customer (stats derived from mv_customer_stats view)
