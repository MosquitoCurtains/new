-- =============================================================================
-- Merge Page Reviews into Site Pages
-- =============================================================================
-- This migration:
-- 1. Adds review columns to site_pages (from migration 4)
-- 2. Migrates any data from page_reviews table (if it exists)
-- 3. Inserts all pages from migration tracker
-- 4. Drops the separate page_reviews table (optional - keeps for safety)
-- =============================================================================

-- Add simple review status enum if not exists
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'simple_review_status') THEN
    CREATE TYPE simple_review_status AS ENUM ('pending', 'complete', 'needs_revision');
  END IF;
END $$;

-- Add review columns to site_pages if they don't exist
...
-- Migrate data from page_reviews if that table exists
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'page_reviews') THEN
    -- Update site_pages with data from page_reviews where slugs match
    -- Cast through TEXT to convert between enum types
    UPDATE site_pages sp
    SET 
      review_status = (pr.status::TEXT)::simple_review_status,
      review_notes = pr.notes,
      revision_items = pr.revision_items,
      reviewed_at = pr.reviewed_at,
      reviewed_by = pr.reviewed_by
    FROM page_reviews pr
    WHERE sp.slug = pr.slug AND pr.status IS NOT NULL;
    
    RAISE NOTICE 'Migrated review data from page_reviews to site_pages';
  END IF;
END $$;
...
DROP VIEW IF EXISTS page_audit_dashboard CASCADE;
...
SELECT 
  ...
  COUNT(*) FILTER (WHERE review_status = 'pending' OR review_status IS NULL) as pending_review,
  COUNT(*) FILTER (WHERE review_status = 'complete') as complete,
  COUNT(*) FILTER (WHERE review_status = 'needs_revision') as needs_revision
FROM site_pages;

DROP VIEW IF EXISTS pages_needing_revision;
CREATE VIEW pages_needing_revision AS
SELECT 
  id, slug, title, page_type, migration_status,
  review_notes, revision_items, reviewed_at, updated_at
FROM site_pages
...
-- =============================================================================
-- CLEANUP: Drop the old page_reviews table and enum
-- =============================================================================

DROP TABLE IF EXISTS page_reviews CASCADE;
